...
const selector = new Map([
	['users', getEl('.users')],
	['stateSelectUser', getEl('.select__user')],
...
function setEvents() {
	...
	selector.get('fieldAddUser').addEventListener('keypress', function(e) {
		...
		if (e.charCode == 13) { addUser(); selector.get('fieldDesc').focus(); }
	});
	...
	getEl('.add__container').addEventListener('keypress', function(e) {
		// if user hits enter in this input field
		if (e.charCode == 13) { handleItemAddition(); }
	});
...
function addUser() {
	if (checkValues('fieldAddUser')) {
		addDataUser(selector.get('fieldAddUser').value, types, true, function(user) {
...
function updateUser(newUser, fromDropDown) {
	...
	fromDropDown ? setActiveUser(newUser) : selectActiveUserFromDropDown(newUser);
	getUserBudget(newUser, function(budget) {
		reCreateItems(budget);
...
function addItem() {
	...
	if (checkValues('fieldType', 'fieldDesc', 'fieldVal')) {
		...
		addDataItem(itemType, domValues, function(budget) {
...
function removeItem(currUser, itemType, id) {
	...
	removeDataItem(currUser, itemType, id, function(budget) {
		// now that we've returned, pass DOM field values into HTML template and add it to the DOM
